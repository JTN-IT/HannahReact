...
const mm__default = /*#__PURE__*/_interopDefaultLegacy(mm);
...
function importGlobPlugin(options = {}) {
  ...
  function getAffectedModules(file) {
    ...
    for (const [id, allGlobs] of map) {
      if (allGlobs.some((glob) => mm__default.isMatch(file, glob)))
...
async function parseImportGlob(code, dir, root, resolveId) {
  ...
  const tasks = matchs.map(async (match, index) => {
    ...
    let lastTokenPos;
    try {
      ast = acorn.parseExpressionAt(code, start, {
        ecmaVersion: "latest",
        sourceType: "module",
        ranges: true,
        onToken: (token) => {
      ...
      try {
        ...
        ast = acorn.parseExpressionAt(" ".repeat(start) + statement, start, {
          ecmaVersion: "latest",
          sourceType: "module",
          ranges: true
        });
...
const { basename, dirname, relative, join } = path.posix;
async function transform(code, id, root, resolveId, { restoreQueryExtension = false, takeover = false } = {}) {
  id = toPosixPath(id);
  ...
  const staticImports = (await Promise.all(matches.map(async ({ globsResolved, isRelative, options, index, start, end }) => {
    ...
    const files = (await fg__default(globsResolved, {
      cwd,
      absolute: true,
    ...
    let query = !options.query ? "" : typeof options.query === "string" ? options.query : ufo.stringifyQuery(options.query);
    ...
    files.forEach((file, i) => {
      ...
      if (options.eager) {
        ...
        const expression = options.import ? `{ ${options.import} as ${variableName} }` : `* as ${variableName}`;
...
async function toAbsoluteGlob(glob, root, dirname2, resolveId) {
  let pre = "";
  if (glob.startsWith("!")) {
    pre = "!";
    glob = glob.slice(1);
  ...
  if (glob.startsWith("/"))
    return pre + path.posix.join(root, glob.slice(1));
  if (glob.startsWith("./"))
    return pre + path.posix.join(dirname2, glob.slice(2));
  if (glob.startsWith("../"))
    return pre + path.posix.join(dirname2, glob);
  if (glob.startsWith("**"))
    return pre + glob;
...
function getCommonBase(globsResolved) {
  const bases = globsResolved.filter((g) => !g.startsWith("!")).map((glob) => {
    let { base } = mm__default.scan(glob);
